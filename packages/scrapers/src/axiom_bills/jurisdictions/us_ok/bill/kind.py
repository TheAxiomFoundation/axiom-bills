...
TITLE_PATTERNS = compile_kind_patterns([
    (r"\bappropriat(?:e|es|ing|ion|ions)\b|\bbudget\b|\bexpenditure\b", BillKind.APPROPRIATIONS),
    (
        r"\bcongratulat(?:e|ing|es)\b|\bcommend(?:ing|s)?\b|\bmemorial\b|"
        r"\bhonor(?:ing|s)?\b|\brecogniz(?:e|ing|es)\b|\bin memoriam\b",
        BillKind.CEREMONIAL,
    ),
    (r"\brules\b|\badjourn(?:ment)?\b|\bsine die\b|\blegislative session\b", BillKind.PROCEDURAL),
])
